# LenoSwap Profile SDK

This package provides some handy functions to retrieve data for Lenoswapswap Profile system.
...
Install `@lenoswap/profile-sdk` into your project with npm:

```bash
npm install @lenoswap/profile-sdk --save
```

or yarn:

```bash
yarn add @lenoswap/profile-sdk
```
...
```js
import LenoswapProfileSdk from "@lenoswap/profile-sdk";

const lenoswapSdk = new LenoswapProfileSdk();
```
...
const lenoswapSdk = new LenoswapProfileSdk(customProvider, 97);
```
...
const lenoswapSdk = new LenoswapProfileSdk();
const username = lenoswapSdk.getUsername("0x123456789");
...
const lenoswapSdk = new LenoswapProfileSdk();
const team = lenoswapSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for lenoswapswap.finance domain, maybe configurable in future versions)

```js
import LenoswapProfileSdk from "@lenoswap/profile-sdk";

const lenoswapSdk = new LenoswapProfileSdk();
const profile = lenoswapSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = lenoswapSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@lenoswap/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [lenoswap-frontend](https://github.com/LenoSwap/frontend) repo. There are several improvements to be made in the future versions of this SDK:
