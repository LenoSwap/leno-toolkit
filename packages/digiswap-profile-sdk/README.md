	# DigiSwap Profile SDK

This package provides some handy functions to retrieve data for DigiSwap Profile system.
...
Install `@digiswap-core/profile-sdk` into your project with npm:

```bash
npm install @digiswap-core/profile-sdk --save
```

or yarn:

```bash
yarn add @digiswap-core/profile-sdk
```
...
```js
import DigiswapProfileSdk from "@digiswap-core/profile-sdk";

const digiswapSdk = new DigiswapProfileSdk();
```
...
const digiswapSdk = new DigiswapProfileSdk(customProvider, 97);
```
...
const digiswapSdk = new DigiswapProfileSdk();
const username = digiswapSdk.getUsername("0x123456789");
...
const digiswapSdk = new DigiswapProfileSdk();
const team = digiswapSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for digiswapswap.finance domain, maybe configurable in future versions)

```js
import DigiswapProfileSdk from "@digiswap-core/profile-sdk";

const digiswapSdk = new DigiswapProfileSdk();
const profile = digiswapSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = digiswapSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@digiswap-core/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [digiswap-frontend](https://github.com/DigiSwap-Core/digiswap-frontend) repo. There are several improvements to be made in the future versions of this SDK:
